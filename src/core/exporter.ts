...
import { formatMapBody, formatAgentGuideMap } from './map-format.js';
...
export async function exportToMarkdown(rootDir: string): Promise<string> {
  ...
  const map = await readMapIfPresent(mapPath);
  if (map) {
    markdown += '## 🗺️ Code Map\n\n';
    markdown += formatMapBody(map);
    markdown += '---\n\n';
  }
...
export async function exportToClaudeMd(rootDir: string): Promise<string> {
  return buildAgentGuide(rootDir, '# CLAUDE.md\n\n');
}

export async function exportToAgentsMd(rootDir: string): Promise<string> {
...
async function readMapIfPresent(mapPath: string): Promise<CodeMap | null> {
  if (!(await fileExists(mapPath))) return null;
  try {
    const map = await readJSON<CodeMap>(mapPath);
    return Array.isArray(map.modules) ? map : null;
  } catch {
    return null;
  }
}

async function buildAgentGuide(rootDir: string, header: string): Promise<string> {
  const contextDir = resolveContextDir(rootDir);
  let md = header;
  ...
  const map = await readMapIfPresent(join(contextDir, CONTEXT_FILES.MAP));
  ...
  if (arch) {
    ...
    if (map && map.modules.length > 0) {
      md += formatAgentGuideMap(map);
    } else if (arch.directories && arch.directories.length > 0) {
      md += '**Key Directories:**\n';
...
export type ExportFormat = 'md' | 'json' | 'claude-md' | 'agents-md' | 'cursorrules';

export async function saveExport(rootDir: string, format: ExportFormat = 'md'): Promise<string> {
  const contextDir = resolveContextDir(rootDir);

  if (format === 'agents-md') {
    const content = await exportToAgentsMd(rootDir);
    const exportPath = join(contextDir, 'AGENTS.md');
